...
impl ApiService {
    ...
    pub fn update(&self, _data:&Data, _param: &String) -> Result<UpdateResult, Error> {
        let object_param: bson::oid::ObjectId = bson::oid::ObjectId::with_string(_param).unwrap();
    ...
    pub fn get_json(&self) -> std::result::Result<std::vec::Vec<bson::ordered::OrderedDocument>, mongodb::error::Error> {
        let cursor: mongodb::Cursor = self.collection.find(None, None).ok().expect("Failed to execute find.");
        let docs: Vec<_> = cursor.map(|doc: Result<bson::ordered::OrderedDocument, Error>| doc.unwrap()).collect();
        Ok(docs)
    }
    ...
    pub fn get_by(&self, param: &String) -> std::result::Result<std::vec::Vec<bson::ordered::OrderedDocument>, mongodb::error::Error> {
        let cursor: mongodb::Cursor = self.collection.find(doc! { "author": { "$regex": param } }, None).ok().expect("Failed to execute find.");
        let docs: Vec<_> = cursor.map(|doc: Result<bson::ordered::OrderedDocument, Error>| doc.unwrap()).collect();
        let _serialized: String = serde_json::to_string(&docs).unwrap();
        Ok(docs)
    }
}